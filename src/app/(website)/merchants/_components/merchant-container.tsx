...
import { MerchantProfileResponse } from "@/types/merchant";
import { useMerchantState } from "@/zustand/merchant";

const MerchantContainer = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const { value, sortBy } = useMerchantState();

  const debounceValue = useDebounce(value);

  const { data, isLoading, isError, error } = useQuery<MerchantProfileResponse>(
    {
      queryKey: ["merchantsProfile", currentPage, debounceValue, sortBy],
      queryFn: () =>
        fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/merchant?page=${currentPage}&limit=6&fullName=${debounceValue}&order=${sortBy}`,
        ).then((res) => res.json()),
    },
  );
  ...
  if (isLoading) {
    ...
    content = (
      <div className="container mb-[40px] grid grid-cols-1 gap-x-[24px] gap-y-[40px] md:grid-cols-2 lg:mb-[104px] lg:grid-cols-3">
        {data.data.map((item) => (
          <MerchantCard data={item} key={item._id} />
        ))}
      </div>
    );
  }
