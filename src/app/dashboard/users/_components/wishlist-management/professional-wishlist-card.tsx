...
const ProfessionalWishlistCard = ({ data }: Props) => {
  const { addProfessional } = useWishlistState();

  if (!data) return;
  return (
    <Link href={`/organizations/${data?.userID}`}>
      <div className="rounded-lg bg-white p-4 transition-all duration-500 hover:scale-[101%]">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-grow">
            ...
            <p className="mt-1 text-sm text-gray-600">{data?.about}</p>
          </div>
        </div>
      </div>
    </Link>
...
export default ProfessionalWishlistCard;
