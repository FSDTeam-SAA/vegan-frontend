...
import OrganizationEventDiolog from "./organization-event-dialog";
...
export function EventCard({ data }: Props) {
  ...
  const { mutate: deleteMutate, isPending: deletePending } = useMutation({
    ...
    mutationFn: () =>
      fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/organizationGoLive/${data?._id}`,
        {
          method: "DELETE",
        },
      ).then((res) => res.json()),
  ...
  return (
    <>
      <Card className="mb-4 border-[#E5E7EB] shadow-none">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 rounded-xl border-[#E5E7EB] bg-white pb-0">
          <Badge
            variant="secondary"
        ...
        <CardContent className="rounded-xl bg-[#F9FAFB] p-0">
          <div className="space-y-4">
            ...
            {isExpanded && (
              <div className="m-6 space-y-4 rounded-xl pb-6">
